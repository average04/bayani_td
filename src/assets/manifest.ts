...
// LPC-generator humanoid sheets. Modern export row blocks: spellcast 0-3, thrust 4-7,
// walk 8-11, slash 12-15, shoot 16-19, hurt 20; direction order per block up/left/down/right.

// 18-column sheets (width 1152, 1188 frames): index = row*18 + col.
function char18(key: string): CharacterAsset {
  return {
    key,
    displayScale: 0.6,
    originY: 0.85,
    anims: {
      idle: { sheet: key, frameRate: 4, repeat: -1, rows: { down: { start: 180, end: 180 } } },
      walk: {
        sheet: key,
        frameRate: 9,
        repeat: -1,
      ...
      death: { sheet: key, frameRate: 10, repeat: 0, rows: { down: { start: 360, end: 365 } } },
    },
  };
}

// 13-column sheets (width 832): the classic universal indices, index = row*13 + col.
function char13(key: string): CharacterAsset {
  return {
    key,
    displayScale: 0.6,
    originY: 0.85,
    anims: {
      idle: { sheet: key, frameRate: 4, repeat: -1, rows: { down: { start: 130, end: 130 } } },
      walk: {
        sheet: key,
        frameRate: 9,
        repeat: -1,
      ...
      death: { sheet: key, frameRate: 10, repeat: 0, rows: { down: { start: 260, end: 265 } } },
    },
  };
}

const lapulapuChar = char18('lapulapu');
const gabrielaChar = char18('gabriela');
const bernardoChar = char13('bernardo');
const diwataChar = char13('diwata');
const mangkukulamChar = char13('mangkukulam');
...
export const MANIFEST: AssetManifest = {
  sheets: [
    ...
    { key: 'gabriela', path: 'assets/sprites/gabriela/sheet.png', frameWidth: 64, frameHeight: 64, frameCount: 1188 },
    // bernardo/diwata/mangkukulam: LPC-generator sheets, 832x3456 -> 13 cols x 54 rows = 702 frames
    { key: 'bernardo', path: 'assets/sprites/bernardo/sheet.png', frameWidth: 64, frameHeight: 64, frameCount: 702 },
    { key: 'diwata', path: 'assets/sprites/diwata/sheet.png', frameWidth: 64, frameHeight: 64, frameCount: 702 },
    { key: 'mangkukulam', path: 'assets/sprites/mangkukulam/sheet.png', frameWidth: 64, frameHeight: 64, frameCount: 702 },
